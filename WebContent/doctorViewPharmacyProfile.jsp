		...
		<div class="col-md-12 column">
			...
			<c:if test="${connect == 0}">
						<div class="col-md-3 column"><div id="${pharmacy.id}">
							 <input class="btn btn-info btn-xs" type="button" style=";" value="Add To My Network" onclick="addp(${pharmacy.id})"></div></div></c:if>
							
							 <c:if test="${connect == 1}">
						
						<div class="col-md-3 column">
							 <a href="order.doctor?id=${pharmacy.id}" class="btn btn-block btn-xs" type="button" >Make Order</a></div>
							 <div class="col-md-3 column">
    ...
    <input type="radio" id="starhalf" name="rating" value="0.5" /><label class="half" for="starhalf" title="Sucks big time - 0.5 stars"></label>
					...
					<c:if test="${not empty reviews}">
							...
							<div class="panel-heading" style="background-color:white;" >
								...
								<c:if test="${review.type == 1}"><c:if test="${review.reviewerId != doctor.id}"><a href="doctorViewDoctor.profile?id=${review.reviewerId}">Dr . ${review.reviewerName}</a></c:if>
								<c:if test="${review.reviewerId == doctor.id}">Your Review</c:if></c:if>
								<c:if test="${review.type == 2}"><a href="doctorViewPatient.profile?id=${review.reviewerId}">${review.reviewerName}</a></c:if>
								<c:if test="${review.type == 3}"><a href="doctorViewPharmacy.profile?id=${review.reviewerId}"> ${review.reviewerName}</a></c:if>
