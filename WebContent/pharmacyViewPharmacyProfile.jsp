...
var pharmacyName = "${pharmacy1.pharmacyName}";
var id = "${pharmacy1.id}";
		...
		<div class="col-md-12" style="background-color:#00b9bc;height:120px;">
			<div class="row">
				...
				<ul class="img-list">
					...
					<img alt="" src="${pharmacy1.image}" class="img-rounded" style="height:130px;width:130px;" onError="this.onerror=null;this.src='image/d.jpg';" /> 
												</li>
												</ul>
			...
			<div class="row clearfix">
				...
				<div class="col-md-8 column col-md-offset-2 col-sm-offset-1 col-xs-offset-1" style="word-wrap:break-word;">
						...
						<div class="panel-body">
							  <div class="row">
							  <c:if test="${pharmacy1.deleted == 0}">
			 <c:if test="${connect == 0}">
						<div class="col-md-3 column"><c:if test="${pharmacy.id ne pharmacy1.id}"><div id="${pharmacy1.id}">
							 <input class="btn  btn-block  btn-xs" type="button"  value="Add" onclick="addp(${pharmacy1.id})"/></div></div></c:if></div></c:if>
							 
							 <c:if test="${connect == 1}">
						
						<div class="col-md-3 column">
							 <a  class="btn  btn-block  btn-xs" type="button" href="order.pharmacy?id=${pharmacy1.id}" >Make Order</a></div>
							 <div class="col-md-3 column">
							 <input class="btn btn-block  btn-xs" type="button"  value="Remove From Network" onclick="removep(${pharmacy1.id})"/></div></c:if>
							 <a class="btn btn-block btn-xs " type="button"  href=javascript:sendPharmacyMessage();>Send Message</a></c:if></div>
							
				   <br>
			 <br>
			 <div class="row pviewpdesc">
              <div class="col-xs-4 col-sm-3 col-md-3  column pviewplabel">	Rating</div>
             <div class="col-xs-8 col-sm-9 col-md-9 column pviewpcontent">${pharmacy1.rating}</div>
            </div>
			<div class="row pviewpdesc">
              <div class="col-xs-4 col-sm-3 col-md-3  column pviewplabel">	Pharmacy Name:</div>
             <div class="col-xs-8 col-sm-9 col-md-9 column pviewpcontent"> ${pharmacy1.pharmacyName}</div>
            </div>		 
	    
	    <div class="row pviewpdesc">
              <div class="col-xs-4 col-sm-3 col-md-3  column pviewplabel">	Delivery Charges:</div>
             <div class="col-xs-8 col-sm-9 col-md-9 column pviewpcontent"> ${pharmacy1.deliveryCharge}Rs.</div>
            </div>
            
            <div class="row pviewpdesc">
              <div class="col-xs-4 col-sm-3 col-md-3  column pviewplabel">	Discount:</div>
             <div class="col-xs-8 col-sm-9 col-md-9 column pviewpcontent"> ${pharmacy1.discount}%</div>
            </div>
            
	    
			<div class="row pviewpdesc">
              <div class="col-xs-4 col-sm-3 col-md-3  column pviewplabel">Pharmacy Address:</div>
             <div class="col-xs-8 col-sm-9 col-md-9 column pviewpcontent">${pharmacy1.landMark}<br>${pharmacy1.address1}<br>${pharmacy1.address2}<br>${pharmacy1.city} - ${pharmacy1.pinCode}<br>${pharmacy1.state}<br>${pharmacy1.country}</div>
            </div>			 
			<div class="row pviewpdesc">
              <div class="col-xs-4 col-sm-3 col-md-3  column pviewplabel">Contact Number:</div>
             <div class="col-xs-8 col-sm-9 col-md-9 column pviewpcontent">${pharmacy1.mobile}</div>
            </div>
            <div class="row pviewpdesc">
              <div class="col-xs-4 col-sm-3 col-md-3  column pviewplabel">Landline Number:</div>
             <div class="col-xs-8 col-sm-9 col-md-9 column pviewpcontent">${pharmacy1.landLine}</div>
            </div>			 		
	   
			<div class="row pviewpdesc">
              <div class="col-xs-4 col-sm-3 col-md-3  column pviewplabel"> Email:</div>
             <div class="col-xs-8 col-sm-9 col-md-9 column pviewpcontent">${pharmacy1.emailId}</div>
            </div>	
           	 
			<div class="row pviewpdesc">
              <div class="col-xs-4 col-sm-3 col-md-3  column pviewplabel">About ${pharmacy.pharmacyName}:</div>
             <div class="col-xs-8 col-sm-9 col-md-9 column pviewpcontent">${pharmacy1.aboutMe}</div>
            </div>						
					</div>
					</div>	
					<div class="container">
    ...
    <input type="radio" id="starhalf" name="rating" value="0.5" /><label class="half" for="starhalf" title="Sucks big time - 0.5 stars"></label>
					...
					<c:if test="${not empty reviews}">
							...
							<div class="panel-heading" style="background-color:white;" >
								...
								<c:if test="${review.type == 3}"><c:if test="${review.reviewerId != pharmacy.id}"><a href="pharmacyViewPharmacy.profile?id=${review.reviewerId}">Dr . ${review.reviewerName}</a></c:if>
								<c:if test="${review.reviewerId == pharmacy.id}">Your Review</c:if></c:if>
								...
								<span class="pull-right glyphicon glyphicon-star" style="color:;"></span></h5></div>
